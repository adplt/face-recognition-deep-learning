...
ground_output = tf.keras.layers.Dense(13, activation='sigmoid', name='ground')(fc_2)
...
# seq = KagglePlanetSequence('./KagglePlanetMCML.csv', './data/train/', im_size=IM_SIZE, batch_size=32)
# callbacks = [tf.keras.callbacks.ModelCheckpoint('./model.h5', verbose=1)]
# model.fit_generator(generator=seq, verbose=1, epochs=1, use_multiprocessing=True, workers=4, callbacks=callbacks)
# another_model = tf.keras.models.load_model('./model.h5')
# another_model.fit_generator(generator=seq, verbose=1, epochs=1)
#
# # Testing
#
# test_seq = KagglePlanetSequence('./KagglePlanetMCML.csv', './data/train/', im_size=IM_SIZE, batch_size=32, mode='test')
